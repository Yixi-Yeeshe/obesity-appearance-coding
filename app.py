...
# New app settings
# =============================
RAW_SHEET = "obesity_appearance"
KAPPA_SHEET = "appearance_kappa"

DATA_PATH = "obesity appearance.csv"
...
st.set_page_config(page_title="Obesity Appearance Coding", layout="wide")
st.title("Obesity Appearance Coding Task")
...
def update_kappa_format(kappa_ws, df):
    """
    Rebuild appearance_kappa from obesity_appearance.
...
st.write("obesity_appearance = 原始长表，每一行是一个 coder 对一个 S 片段的判断。")
st.write("appearance_kappa = 可用于计算 kappa 的宽表。")

if st.button("手动更新 appearance_kappa 表"):
    latest_df = read_raw_data_cached()
    update_kappa_format(kappa_ws, latest_df)
    st.success("appearance_kappa 已更新。")
